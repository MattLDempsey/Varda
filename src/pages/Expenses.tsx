...
export default function Expenses() {
  ...
  const s: Record<string, CSSProperties> = {
    ...
    barLabel: { fontSize: 13, color: C.silver, width: 140, textAlign: 'right' as const, flexShrink: 0 },
    barTrack: { flex: 1, height: 24, background: C.black, borderRadius: 6, overflow: 'hidden' },
    barAmount: { fontSize: 13, color: C.white, fontWeight: 500, width: 100, flexShrink: 0 },
    // Panel
    overlay: { position: 'fixed' as const, inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 90 },
    panel: {
      position: 'fixed' as const, top: 0, right: 0, bottom: 0, width: 440, maxWidth: '100vw',
      background: C.charcoal, borderLeft: `1px solid ${C.steel}44`, zIndex: 100,
      overflowY: 'auto' as const, padding: '28px 28px 40px', boxShadow: '-8px 0 32px rgba(0,0,0,.5)',
    },
